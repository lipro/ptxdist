...
ptxd_make_xilinx_bsp_apply_uboot()
{
    ...
    echo "xlbsp: ${pkg_xlbsp_apply}: board '$(ptxd_print_path "${ptx_xlbsp_uboot_dir:-<none>}")'"
    pushd "${ptx_xlbsp_uboot_dir}" > /dev/null &&
    for pkg_xlbsp_uboot_file in ${pkg_xlbsp_uboot_files}; do
	echo "xlbsp: ${pkg_xlbsp_apply}: rm/ln '$(ptxd_print_path "${pkg_xlbsp_uboot_file:-<none>}")'"
...
ptxd_make_xilinx_bsp_apply_kernel()
{
	...
	*)
    ...
    # apply files (remove old, link new)
    echo "xlbsp: ${pkg_xlbsp_apply}: board '$(ptxd_print_path "${ptx_xlbsp_kernel_dir:-<none>}")'"
    pushd "${ptx_xlbsp_kernel_dir}" > /dev/null &&
    for pkg_xlbsp_kernel_file in ${pkg_xlbsp_kernel_files}; do
	echo "xlbsp: ${pkg_xlbsp_apply}: rm/ln '$(ptxd_print_path "${pkg_xlbsp_kernel_file:-<none>}")'"
